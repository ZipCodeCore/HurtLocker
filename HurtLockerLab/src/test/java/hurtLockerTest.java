...
public class hurtLockerTest {
    ...
    public void splitSortTest() {
        int expected = 8;
        hurtLocker.splitSort(input);
        int actual = hurtLocker.milkTally1;
        Assert.assertEquals("Values should be equal because splitSort should split values and then sort then into appropriate arrays", expected, actual);
    }
    ...
    hurtLocker.splitSort(input);
    HashMap milkMap = hurtLocker.priceTally(hurtLocker.milkPrices);
    String actual = (String) milkMap.get("price1");
    Assert.assertEquals("Values should match because priceTally records and counts prices", expected, actual);
    }

    @Test
    public void formatToPrintTest() {
    //Can't test print
    }
}
